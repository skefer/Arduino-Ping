...
void ICMPEcho::serialize(uint8_t * binData) const {
    ...
    *(uint16_t *)binData = htons(icmpHeader.checksum);
    binData += 2;
    *(uint16_t *)binData = htons(id);
    binData += 2;
    *(uint16_t *)binData = htons(seq);
    binData += 2;
    *(time_t *)  binData = htonl(time);
    binData += 4;
...
void ICMPEcho::deserialize(uint8_t const * binData) {
    ...
    icmpHeader.checksum = ntohs(*(uint16_t *)binData);
    binData += 2;
    id                  = ntohs(*(uint16_t *)binData);
    binData += 2;
    seq                 = ntohs(*(uint16_t *)binData);
    binData += 2;

    if(icmpHeader.type != TIME_EXCEEDED) {
        time = ntohl(*(time_t *)binData);
        binData += 4;
    }
...
ICMPEchoReply ICMPPing::Ping(const IPAddress& addr) {
    ICMPEchoReply reply;
    ...
    _id = (uint16_t)random(0, 255);
    operator()(addr, 1, reply);
    return reply;
}
